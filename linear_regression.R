...
#Download data make them stationary for x matrix
...
for (i in 1:n_iter) {
  ...
  alpha_param <- (gamma^2 + b[ncol(X)+1]^2) # alpha parameter
  z <- rgig(n = length(y), lambda = -1, chi = (delta^2) * q_val, psi = alpha_param) # Sample z from GIG distribution
  
  # d,D matrices and Σbar Σ-1
  A1 <- cbind(X, z) # Combine X and z for the regression
  w <- as.vector(1/z)
  A1w <- A1 * w
  D_inv <- t(A1w) %*% A1 + prior_prec_beta # D^-1 matrix
  D <- solve(D_inv) # D matrix
  d_vec <- t(A1) %*% (w*y) + prior_prec_beta %*% prior_mean_beta # d vector
  b <- mvrnorm(1, mu = D %*% d_vec, Sigma = D) # Sample beta coefficients from multivariate normal distribution
...
beta_post <- beta_store[(burnin+1):n_iter, ]
gamma_post <- gamma_store[(burnin+1):n_iter]
delta_post <- delta_store[(burnin+1):n_iter]
z_post_mean <- colMeans(z_store[(burnin+1):n_iter, ]) # Calculate mean z values across iterations
...
rownames(beta_summary) <- c("Intercept", "Skewness (Beta)")
